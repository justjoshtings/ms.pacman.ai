...
class PostProcessor:
  ...
  def broadcast_ready(self, broadcast_dimensions, observation, n_frame, fps, action, action_controls):
    '''
    Method to perform post-processing on observation game state images to be broadcast ready.

    Params:
      self: instance of object
      broadcast_dimensions (tuple) : (width, height)
      observation (np.array) : game state observation
      n_frame (int) : current frame number in episode
      fps (int) : most recent steps' fps
      action (int) : action taken by agent from action space
      action_controls : action_controls (dict) : {action (str) : control image (np.array)}

    Returns:
      observation_broadcast(np.array) : game state observation with post-processing
    '''

    game_size = tuple([int(broadcast_dimensions[0]*(0.7)), int(broadcast_dimensions[1]*1)])
    controller_size = tuple([int(broadcast_dimensions[0]*(0.3)), int(broadcast_dimensions[1]*1)])

    observation_broadcast = observation
    observation_broadcast = cv2.resize(observation_broadcast, dsize=game_size, interpolation=cv2.INTER_AREA)
    ...
    observation_broadcast = cv2.putText(observation_broadcast,
                                ...
                                org=(48,int(game_size[1]*0.93)), 
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX, 
                                fontScale=0.4,
                                color=(255,255,255), #BGR
                                thickness=1)
    
    controller_state = cv2.resize(action_controls[action], dsize=controller_size, interpolation=cv2.INTER_AREA)
    observation_broadcast = cv2.hconcat([controller_state,observation_broadcast])
